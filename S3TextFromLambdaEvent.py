...
def get_file_text_from_s3_file_url(s3_file_url):
	"""Returns the text of a single S3 file based on the URL"""
	s3_boto = boto3.resource("s3")
	bucket = get_bucket_name_from_url(s3_file_url)
	key = get_key_from_url(s3_file_url)
	obj = s3_boto.Object(bucket, key)
	file_contents = obj.get()['Body'].read().decode('utf-8') 	
